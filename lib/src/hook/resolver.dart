/// Shared asset resolver for the native build hook and the web setup.
///
...
final _log = Logger('icu_kit:resolver');
...
class ResolveRequest {
  ...
  /// GitHub Release asset name (e.g. `macos-arm64-libicu_capi.dylib`
  /// or `wasm-icu4x.wasm`). Used as the hash lookup key.
...
Future<bool> _resolveFromSource(ResolveRequest req) async {
  ...
  throw StateError(
    ...
    'Options:\n'
    '  1. Use a published version: icu_kit: ^X.Y.Z (downloads a\n'
    '     hash-verified pre-built binary — check your network)\n'
    '  2. Use a git tag with binaries: ref: vX.Y.Z\n'
    '  3. Clone the repo with --recursive and use a path dependency\n'
    '     (compiles from the vendored ICU4X source; needs Rust)\n',
  );
}
...
/// Check if the vendored ICU4X FFI crate exists on disk.
bool hasVendorSource(Uri packageRoot) {
  return File.fromUri(
    packageRoot.resolve('vendor/icu4x/ffi/capi/Cargo.toml'),
  ).existsSync();
}
