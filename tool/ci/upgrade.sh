...
#   build.json (baseTag, features, manual — icu4x fork facts + the pinned Rust
#     nightlyToolchain, outputs)      nightly; move only with the submodule
#
# JSON is parsed with jq — gh's embedded `--jq` for gh-api responses, jq for
# curl'd manifests. In-place version bumps go through set_kv (env-carried
# values, tmp+mv) to preserve the file's formatting. No python, no dart.
#
...
command -v jq >/dev/null 2>&1 || { echo "upgrade: jq not found (needed to parse manifests)" >&2; exit 2; }

# sha256 of a file, coreutils or BSD flavor. Stdin, never a path argument:
# GNU coreutils escapes the output line when the filename carries a
# backslash (every Windows Git Bash path does); stdin keeps it clean.
sha256_file() {  # path
...
# masquerade as the empty-string hash. Large assets get a longer window than
# _fetch's 30s manifest budget: stall-bounded (under 2KB/s for 30s), with
# --max-time as the hung-pipe backstop.
sha256_of() {  # url
  local tmp; tmp="$(mktemp)"
  if curl -fsSL --retry 3 --retry-delay 2 --max-redirs 5 --connect-timeout 10 \
       --speed-limit 2048 --speed-time 30 --max-time 1800 "$1" -o "$tmp" 2>/dev/null \
     && [ -s "$tmp" ]; then
    sha256_file "$tmp"
  fi
  rm -f "$tmp"
