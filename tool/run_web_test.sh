#!/usr/bin/env bash
# ────────────────────────────────────────────────────────────────────
# run_web_test.sh — Run the flutter drive web integration test.
#
# Uses -d web-server: Flutter serves the app, chromedriver manages
# one Chrome instance. No DWDS, no dual-Chrome, no AppConnection
# race, and --no-sandbox reaches Chrome (a bare `-d chrome` launch
# can't pass it, which kills Chrome's zygote on hardened CI runners).
#
# Usage:
#   run_web_test.sh <flutter_cmd...>
#
#   flutter_cmd  e.g. "fvm flutter"
#
# Called by:  Makefile target (test-example-web)
...
if grep -q 'All tests passed' "$LOG"; then
    echo "=== Example web: All tests passed ==="
    exit 0
else
    echo "=== Example web: FAILED ==="
    cat "$LOG"
    exit 1
fi
