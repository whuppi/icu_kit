...
//
// ── icu_kit patch ── grouping_strategy constructor param threaded into the inner DecimalFormatter (ECMA-402 useGrouping).
...
impl LongCurrencyFormatter {
    icu_provider::gen_buffer_data_constructors!(
        (prefs: CurrencyFormatterPreferences, currency_code: &CurrencyCode, grouping_strategy: Option<icu_decimal::options::GroupingStrategy>) -> error: DataError,
        functions: [
            try_new: skip,
    ...
    /// let fmt = LongCurrencyFormatter::try_new(currency_preferences, &currency_code, None).unwrap();
