...
pub struct CurrencyFormatterOptions {
    ...
    /// Grouping strategy forwarded to the inner `DecimalFormatter`
    /// (`None` = the formatter's default, i.e. locale-aware `Auto`).
    /// icu_kit patch for ECMA-402 `useGrouping`.
    #[cfg_attr(feature = "serde", serde(skip))]
    pub grouping_strategy: Option<icu_decimal::options::GroupingStrategy>,
}
...
pub enum Width {
    ...
    /// Format the currency with its 3-letter ISO 4217 code (CLDR `¤¤`).
    ///
    /// For example, 1 USD formats as "USD 1.00" in en-US. Uses the ISO code as
    /// the placeholder with the alpha-next-to-number pattern — the spacing
    /// CLDR applies for alphabetic currency displays. (icu_kit patch
    /// for ECMA-402 `currencyDisplay: "code"`.)
