...
//
// ── icu_kit patch ── Width::Code maps to the standard_alpha_next_to_number pattern with an ISO-code placeholder (ECMA-402 currencyDisplay: code).
...
impl<'a> CurrencyEssentials<'a> {
    ...
    pub(crate) fn name_and_pattern(
        ...
        let placeholder_index = match width {
            ...
            // Code display: force the ISO-code fallback (no symbol placeholder).
            Width::Code => None,
        };
        ...
        let pattern_selection = match width {
            ...
            // Code is alphabetic → the alpha-next-to-number pattern (adds the
            // space CLDR puts between an alphabetic display and the number).
            Width::Code => PatternSelection::StandardAlphaNextToNumber,
        };
