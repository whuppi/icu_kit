...
pub mod ffi {
    ...
    impl<'info> BidiParagraph<'info> {
        ...
        // ── icu_kit patch ── paragraph + reordered levels for UCD BidiCharacterTest
        ...
        #[diplomat::rust_link(unicode_bidi::ParagraphInfo::level, StructField)]
        #[diplomat::attr(auto, getter)]
        pub fn level(&self) -> u8 {
            self.0.para.level.number()
        }
        ...
        #[diplomat::rust_link(
            unicode_bidi::BidiInfo::reordered_levels_per_char,
            FnInStruct
        )]
        pub fn reordered_level_at(&self, pos: usize) -> u8 {
            let para = self.0.para;
            let levels = self
                .0
                .info
                .reordered_levels_per_char(para, para.range.clone());
            let local = pos.saturating_sub(para.range.start);
            levels.get(local).map(|l| l.number()).unwrap_or(0)
        }
        // ── end icu_kit patch ──
